...
namespace LinkedList
{
    ...
    // Simple method for finding address of middle node
    // Counts the number of elements, ceils the value and positions the pointer
    // at the ceiled value index.
    // Time complexity -> o(n)
    // Space complexity -> o(1)
    [[nodiscard]] std::shared_ptr<Node> LinkedList::findMiddleNodeSimple() const
    {
        unsigned int nodeCount = countNodes();
        unsigned int middleNodeIdx = ceil(countNodes() / 2);
        std::shared_ptr<Node> middleNode = head;

        for (unsigned int i = 0; i < middleNodeIdx; ++i)
        {
    ...
    // First pointer moves for two places, while the second moves only one place
    // at the time. When the first pointer becomes nullptr, second pointer
    // will travel half the distance and will be positioned at the middle of the linked list.
    // Time complexity -> o(n)
    // Space complexity -> o(1)
    [[nodiscard]] std::shared_ptr<Node> LinkedList::findMiddleNode() const
    {
        std::shared_ptr<Node> first = head;
        std::shared_ptr<Node> middleNode = head;
    ...
    // This method will store the addresses of the Linked List nodes into a stack.
    // Than the stack will be counted and popped for the n / 2 ammount of times.
    // The stack head will point to the middle node.
    // Time complexity -> o(n)
    // Space complexity -> o(n) -> we need auxilary space - stack
    [[nodiscard]] std::shared_ptr<Node> LinkedList::findMiddleNodeAdvanced() const
    {
        std::stack<std::shared_ptr<Node>> nodeStack;
        std::shared_ptr<Node> node = head;
        std::shared_ptr<Node> middleNode = nullptr;
        unsigned int stackSize = 0;
        unsigned int middlePosition = 0;
        ...
        if (stackSize != 0U)
        {
            middlePosition = ((stackSize % 2) == 0) ? ((stackSize / 2) - 1) : ceil(stackSize / 2);

            for (unsigned int i = 0; i < middlePosition; ++i)
            {
                nodeStack.pop();
            }

            middleNode = nodeStack.top();
        }
